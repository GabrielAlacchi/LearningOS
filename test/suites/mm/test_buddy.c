...
static void check_free_integrity(buddy_allocator_t *allocator) {
    ...
    assert_int_equal(allocator->freelist_pool_size - used_freelist_count - 1, allocator->freelist_space_left);
...
static int setup_buddy_pool(void **state) {
    ...
    pool.freelist_pool = malloc(vector.freelist_pool_pages << PAGE_ORDER);
    pool.freelist_pool_pages = vector.freelist_pool_pages;
    return 0;
}
...
static int teardown_buddy_pool(void **state) {
    ...
    free(pool.freelist_pool);
    return 0;
}


static void test_buddy_init(void **state) {
    buddy_allocator_t *allocator = buddy_init(pool, 0x1000, PHYS_MEM_SIZE);
...
static void test_block_splitting_and_coalescing(void **state) {
    ...
    while (freelist->flags & FREELIST_PRESENT) {
        ++order_0_free;
        freelist = &allocator->freelist_pool[freelist->next_entry];
    }
    ...
    assert_false(allocator->freelists[0]->flags & FREELIST_PRESENT);

    u8_t next_order = 1;
    while (!(allocator->freelists[next_order]->flags & FREELIST_PRESENT)) {
        ++next_order;
    }
    ...
    for (u8_t order = 0; order < next_order; ++order) {
        ...
        assert_true(allocator->freelists[order]->flags & PT_PRESENT);
    ...
    for (u8_t order = 0; order < next_order; ++order) {
        assert_false(allocator->freelists[order]->flags & PT_PRESENT);
    }

    assert_true(allocator->freelists[next_order]->flags & PT_PRESENT);
...
static void test_block_shrinking(void **state) {
    ...
    assert_true(freelist->flags & FREELIST_PRESENT);
